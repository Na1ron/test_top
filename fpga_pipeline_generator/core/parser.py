...
class ConfigParser:
    ...
    def parse_all_submodules(self, target_stages: List[str]) -> Dict[str, Dict[str, List[Dict[str, Any]]]]:
        """
            ...
            for stage in target_stages:
                ...
                if targets:
                    ...
                    for target_config in targets:
                        target_name, variables, options = self.extract_target_info(target_config)
    ...
    def validate_stages(self, stages: List[str], supported_stages: List[str]) -> List[str]:
        ...
        for stage in stages:
            if stage in supported_stages:
                ...
                print(f"Предупреждение: стадия '{stage}' не поддерживается. Поддерживаемые: {supported_stages}")
